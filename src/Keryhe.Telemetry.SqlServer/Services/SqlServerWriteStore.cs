...
/// <summary>
/// SqlServer implementation of <see cref="ITelemetryWriteStore"/> — the write-path
/// <c>Delete*</c> operations. Child rows (span events/links, metric data points) are
/// removed by the schema's <c>ON DELETE CASCADE</c> foreign keys, so the deletes target
/// only the parent tables.
/// </summary>
