    ...
    ILogger<TimescaleBulkWriter> logger) : ITelemetryBulkWriter
{
        ...
        CancellationToken ct)
    {
        const string sql = """
            ...
            -- ON CONFLICT names all three columns of uk_trace_span, widened in schema 2.11.0 to
            -- include start_time_unix_nano: spans is a hypertable now, and TimescaleDB only
            -- supports ON CONFLICT against a constraint that includes the partition column.
            ON CONFLICT (trace_id, span_id, start_time_unix_nano) DO NOTHING
            """;
